...
team = {
  rafael: User.create!(email: 'rafael@agencios.app', password: 'demo1234', name: 'Rafael Lima',
                       confirmed_at: Time.current),
  julia: User.create!(email: 'julia@agencios.app',  password: 'demo1234', name: 'Júlia Santos',
                      confirmed_at: Time.current),
  pedro: User.create!(email: 'pedro@agencios.app',  password: 'demo1234', name: 'Pedro Alves',
                      confirmed_at: Time.current),
  bia: User.create!(email: 'bia@agencios.app', password: 'demo1234', name: 'Bia Rocha',
                    confirmed_at: Time.current)
}
...
# ── Clients + projects ───────────────────────────────────────────────
client_data = [
  { name: 'Açaí da Vila',     company: 'Vila Foods Ltda',  email: 'contato@acaidavila.com.br',
    phone: '(11) 98888-1010', color: '#7C3AED', secondary: '#F59E0B', voice: 'Jovem, vibrante e saudável — tom de verão, gírias leves.' },
  { name: 'GymFit Academia',  company: 'GymFit S.A.',      email: 'mkt@gymfit.com.br',
    phone: '(11) 97777-2020', color: '#10B981', secondary: '#111827', voice: 'Motivacional e direto — energia de treino, foco em resultado.' },
  { name: 'Bloom Cosméticos', company: 'Bloom Beauty',     email: 'social@bloom.com.br',
    phone: '(21) 96666-3030', color: '#EC4899', secondary: '#9D174D', voice: 'Delicada, sofisticada e acolhedora — beleza com autocuidado.' },
  { name: 'TechNova',         company: 'TechNova Startup', email: 'growth@technova.io',
    phone: '(11) 95555-4040', color: '#0EA5E9', secondary: '#1E293B', voice: 'Inovadora e confiante — linguagem de produto, clara e objetiva.' },
  { name: 'Vinho & Cia',      company: 'Adega Premium',    email: 'marketing@vinhoecia.com',
    phone: '(51) 94444-5050', color: '#F43F5E', secondary: '#7F1D1D', voice: 'Elegante e sensorial — apreciação, harmonização, exclusividade.' }
]
...
clients.each do |client, color|
  project_names[client.name].each_with_index do |pname, _i|
...
CHANNELS_POOL = [%w[instagram tiktok], %w[instagram], %w[instagram facebook], %w[instagram youtube], %w[tiktok],
                 %w[instagram linkedin]].freeze
...
tickets = ticket_blueprints.each_with_index.map do |(title, status, ctype), idx|
  ...
  fields = {
    'ideation' => { 'brief' => "Brief de #{title}.", 'objective' => %w[Awareness Engajamento Conversão].sample,
  ...
  if idx.even?
    Note.create!(workspace: ws, ticket: t, user: assignee, kind: :comment,
                 body: 'Bora alinhar o gancho antes de produzir? 🔥')
  end
...
SUBTASK_TITLES = ['Escrever roteiro', 'Definir trilha sonora', 'Gravar cenas', 'Editar corte final',
                  'Revisar legenda', 'Aprovar com cliente', 'Selecionar hashtags', 'Exportar nos formatos'].freeze
tickets.each do |t|
...
tickets.each do |t|
  ...
  creative = Creative.create!(workspace: ws, ticket: t, creative_type: t.creative_type,
                              ...
                              metadata: { 'slides' => (1..rand(4..7)).map do |i|
                                { 'index' => i, 'image_url' => "https://picsum.photos/seed/ag#{t.id}#{i}/600/750", 'headline' => "Slide #{i}" }
                              end })
