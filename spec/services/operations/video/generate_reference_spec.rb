...
# Generates a consistency-anchor reference image (character/scenario) via the
# OpenRouter image model, charged as an image generation; best-effort (never
# fails the video).
RSpec.describe Operations::Video::GenerateReference do
  ...
  it 'generates via OpenRouter, charges an image credit, stores it, and returns { url, role }' do
    ...
    expect(Vendors::OpenRouter::Actions::GenerateImage).to have_received(:call)
      .with(hash_including(aspect_ratio: '1:1'))
  end

  it 'refunds and returns nil when generation fails (never blocks the video)' do
    allow(Vendors::OpenRouter::Actions::GenerateImage).to receive(:call).and_raise(StandardError, 'boom')
