...
# generation params: a resolved track from the open base + clamped ffmpeg params.
RSpec.describe Operations::Video::ResolveMusic do
  ...
  it 'searches Jamendo and carries the orchestrator mix params (clamped)' do
    allow(Vendors::Jamendo::Actions::SearchTracks).to receive(:call).and_return(
      { url: 'https://jamendo/t.mp3', title: 'Track', attribution: 'Track — Artist' }
    )

    out = described_class.call(spec: { 'query' => 'upbeat corporate', 'mood' => 'upbeat',
                                       'volume' => 5.0, 'fade_in' => 1.0, 'fade_out' => 2.0, 'duck' => true })

    expect(Vendors::Jamendo::Actions::SearchTracks).to have_received(:call).with(query: 'upbeat corporate', tags: 'upbeat')
    expect(out).to include('music_url' => 'https://jamendo/t.mp3', 'music_mood' => 'upbeat',
                           'music_volume' => 0.6, # clamped from 5.0
                           'music_fade_in' => 1.0, 'music_fade_out' => 2.0, 'music_duck' => true)
  end

  it 'falls back to the admin catalog when Jamendo finds nothing' do
    allow(Vendors::Jamendo::Actions::SearchTracks).to receive(:call).and_return(nil)
