...
RSpec.describe 'Autopilot (GO mode) API', type: :request do
  ...
  it 'blocks the start with 402 when the wallet is short of the estimate' do
    # Video is never auto-generated in GO (costs 0), so the shortfall must come from
    # a metered non-video type — a feed_image against an empty wallet.
    image = build_ticket(%w[feed_image]) # wallet empty

    post "/api/v1/tickets/#{image.id}/autopilot_start", params: { mode: 'scheduled' }, as: :json

    expect(response).to have_http_status(:payment_required)
    body = JSON.parse(response.body)
    expect(body['code']).to eq('insufficient_credits')
    expect(body['required']).to eq(Pricing.credits_for(kind: :image))
  end
