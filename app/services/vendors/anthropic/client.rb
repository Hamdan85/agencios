  ...
  module Anthropic
    ...
    class Client < Vendors::Base
      ...
      def stream(system:, messages:, tools: [], max_tokens: 2048, on_tool_start: nil, &block)
        return stub_stream(system: system, messages: messages, tools: tools, on_tool_start: on_tool_start, &block) if @api_key.blank?
      ...
      def apply_stream_event(event, state, &block)
        ...
        when "content_block_stop"
          ...
          if state[:tool_meta][idx]
            parsed = JSON.parse(state[:tool_buffers][idx].to_s) rescue nil
