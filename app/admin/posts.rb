...
ActiveAdmin.register Post do
  ...
  index do
    ...
    column :scheduled_at
    column :published_at
    column("Permalink") { |p| p.permalink.present? ? link_to("abrir", p.permalink, target: "_blank", rel: "noopener") : "—" }
    column :created_at
  end
  ...
  show do
    attributes_table do
      ...
      row("Permalink") { |p| p.permalink.present? ? link_to(p.permalink, p.permalink, target: "_blank", rel: "noopener") : "—" }
      row :failure_reason
      row :created_at
    end
  end
