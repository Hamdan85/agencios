...
ActiveAdmin.register PricingPlan do
  ...
  index do
    ...
    column("Créditos inclusos") { |p| p.included_credits }
    column :seats
    column :clients
    column("Stripe lookup") { |p| p.stripe_lookup_key }
    column("Stripe price") { |p| p.stripe_price_id }
    column :active
    actions
  end
  ...
  form do |f|
    ...
    f.inputs "Stripe (fonte da verdade do valor cobrado)" do
      ...
      f.input :stripe_lookup_key, hint: "lookup_key do Price MENSAL. Troque o preço criando um Price novo com transfer_lookup_key."
