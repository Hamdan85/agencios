...
ActiveAdmin.register Workspace do
  ...
  index do
    ...
    column("Acesso") { |w| status_tag(w.billing_active? ? "sim" : "bloqueado", class: w.billing_active? ? "yes" : "error") }
  ...
  member_action :invite, method: :post do
    ...
    if email.blank?
      redirect_to(invite_form_admin_workspace_path(resource), alert: "Informe um e-mail.") && return
    end
    ...
    InvitationMailer.invite(email: email, role: role, link: link, workspace: resource, inviter: current_staff_user).deliver_later
