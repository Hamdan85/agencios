...
// never stored. Pasting a comma/newline-separated list adds them all.
export function ChipsInput({ value, onChange, placeholder, prefix = '', className }) {
  const chips = Array.isArray(value) ? value : []
  const [text, setText] = useState('')

  const clean = (raw) => raw.split(/[,\n]/).map((s) => s.trim().replace(new RegExp(`^${prefix}+`), '').trim()).filter(Boolean)
