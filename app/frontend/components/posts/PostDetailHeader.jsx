    ...
    >
      ...
      <div className="relative">
        ...
        <h1 className="mt-3 font-display text-2xl font-extrabold leading-tight tracking-tight text-ink sm:text-3xl">
          {post.ticket_title || post.campaign_name || t('detail.fallbackTitle')}
        </h1>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          {post.client_id && (
            <EntityChip to={`/clientes/${post.client_id}`}>
              ...
              <span className="max-w-[10rem] truncate">{post.client_name || t('detail.clientFallback')}</span>
            </EntityChip>
          )}
          ...
          {post.campaign_id && (
            <EntityChip to={`/campanhas/${post.campaign_id}`}>
              ...
              <span className="max-w-[12rem] truncate">{post.campaign_name || t('detail.campaignFallback')}</span>
            </EntityChip>
          )}
          ...
          {post.permalink && (
            <EntityChip href={post.permalink}>
              ...
              {t('detail.viewOn', { network: net.label })}
            </EntityChip>
          )}
        </div>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1.5">
          {post.scheduled_at && (
            <TimeLine icon={CalendarClock} color="#F59E0B" label={t('detail.scheduledFor', { date: dt(post.scheduled_at) })} />
          )}
          {post.published_at && (
            <TimeLine icon={Radio} color="#10B981" label={t('detail.publishedAgo', { time: timeAgo(post.published_at) })} />
          )}
          {post.unpublished_at && (
            <TimeLine icon={EyeOff} color="#8B86A3" label={t('detail.unpublishedAgo', { time: timeAgo(post.unpublished_at) })} />
          )}
        </div>
        ...
        {post.status === 'failed' && (
          <div className="mt-4 flex items-start gap-2.5 rounded-xl border border-danger/25 bg-danger/8 p-3.5">
            ...
            <div className="min-w-0">
              <p className="text-sm font-bold text-danger">{t('detail.failed')}</p>
              {post.failure_reason && (
