...
# View helpers for the server-rendered (SSR) marketing pages.
#
# The SPA uses `lucide-react` for icons; these pages are plain ERB, so we inline
# the same Lucide glyphs as SVG. Paths are copied verbatim from Lucide (ISC) and
# rendered with the canonical Lucide attributes (24×24, currentColor stroke).
module PagesHelper
  # Lucide icon path data, keyed by the lucide icon name.
  ICON_PATHS = {
    ...
    # ── Brand-ish glyphs for the network marquee (Lucide simple-brand set) ──
    ...
    'twitter-x' => %(<path d="M4 4l11.733 16H20L8.267 4z"/><path d="M4 20 10.768 12.232M13.232 11.768 20 4"/>)
  }.freeze

  # Render a Lucide icon as inline SVG (currentColor, 24×24 viewBox).
  def icon(name, size: 24, stroke: 2, css: nil)
    paths = ICON_PATHS[name.to_s] || ''
    size = size.to_i
    klass = css ? %( class="#{ERB::Util.html_escape(css)}") : ''
    <<~SVG.html_safe
      <svg xmlns="http://www.w3.org/2000/svg" width="#{size}" height="#{size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="#{stroke}" stroke-linecap="round" stroke-linejoin="round"#{klass} aria-hidden="true">#{paths}</svg>
    SVG
  end

  # A tinted soft-color background (10% of a hex) for an icon badge.
  def soft_bg(hex)
    "#{hex}16"
  end

  # Format an amount of cents as BRL, dropping ",00" for whole values.
  # 9_900 → "R$ 99" · 129_900 → "R$ 1.299" · 9_990 → "R$ 99,90"
  def brl_short(cents)
    reais  = cents / 100
    rest   = cents % 100
    grouped = reais.to_s.reverse.gsub(/(\d{3})(?=\d)/, '\\1.').reverse
    rest.zero? ? "R$ #{grouped}" : format('R$ %s,%02d', grouped, rest)
  end
  ...
  def nav_section_active?(prefix)
    request.path == prefix || request.path.start_with?("#{prefix}/")
  end

  # ── SEO: JSON-LD structured data for the marketing site ────────────
  # Rendered once in the marketing <head>. Describes the Organization and
  # the SaaS product (with a low-price offer pulled from the canonical
  ...
  def marketing_structured_data
    ...
    content_tag(:script, JSON.generate(payload).html_safe, type: 'application/ld+json')
  end
end
