# frozen_string_literal: true

class ProjectSerializer < ActiveModel::Serializer
  attributes :id, :name, :description, :color, :status, :starts_on, :ends_on,
             :budget_cents, :client_id, :client_name, :client_email, :tickets_count,
             :completed_at, :latest_report_id, :settings, :created_at

  def starts_on = object.starts_on&.iso8601
  def ends_on = object.ends_on&.iso8601
  def completed_at = object.completed_at&.iso8601
  def client_name = object.client&.name
  def client_email = object.client&.email
  def tickets_count = object.tickets.count
  def latest_report_id = object.latest_report&.id
  def settings = object.resolved_settings
  def created_at = object.created_at&.iso8601
end
